...
- Fixed a graphics bug that causes `par` not reset after showing
  surfaces
- Added a helper function to convert volume atlases to surface with
  proper smoothing
...
- Improved `plot.ieegio_volume` intensity windowing using
  percentile-based display range (2-98 percentiles), following
...
- `LazyH5` fall back to alternatives when `HDF5` is not supported in
  `wasm`
...
- Writing to `freesurfer` surfaces also appends orientation information
  at the end.
...
- Added merge support for both volume and surface objects; for surface
  objects, users can choose whether to merge the objects in model or
  world space
- Added dependency `ravetools` to help speed up sample volumes using
  `C++`
- Fixed missing transform issue in writing to `GIfTI` format
- Fixed `write_surface` not working when writing to `FreeSurfer`
  measurement (curvature) format
...
- Added `resample_volume` to sample images (currently only nearest
  neighbor)
- Added `burn_volume` to burn contacts (positions given in the image
  native `RAS` coordinate) with any given colors
- Fixed `io_h5_names` returning wrong names when using with `h5py`
- Exported internal class `LazyH5` for down-stream packages
- Added `as_ieegio_volume` and `as_ieegio_surface` to enclose volume or
  surface data
...
- Fixed a bug where images are not loaded correctly due to image format
  being ignored
